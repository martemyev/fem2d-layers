...
double rhs_function(const Point &p, double t)
{
  const double x = p.coord(0);
  const double y = p.coord(1);
  const double h = 10; //param.SOURCE_SUPPORT; // some constant
  const double f = 50; //param.SOURCE_FREQUENCY; // frequency, Hz
  const double xc = 500; //param.SOURCE_CENTER_X; // x-coordinate of the source center
  const double yc = 500; //param.SOURCE_CENTER_Y; // y-coordinate of the source center
